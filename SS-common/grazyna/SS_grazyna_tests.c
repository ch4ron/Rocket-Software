...
extern uint32_t SS_software_crc(uint32_t *data, int len);
...
#include "mocks/MockSS_grazyna_hal.h"
#include "stdio.h"

TEST(grazyna, mock) {
    ComFrame frame = {1, 2, 3};
    uint8_t buff[1] = {0x03};
    SS_grazyna_transmit_ExpectWithArray(buff, 1, sizeof(GrazynaFrame));
