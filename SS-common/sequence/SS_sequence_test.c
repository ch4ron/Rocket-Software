...
static void check_sequence_values(uint8_t item, uint8_t id, uint8_t operation, int16_t value, int16_t time) {
...
TEST(sequence, add_single) {
    SS_sequence_add(1, 2, 3, 4);
    check_sequence_values(0, 1, 2, 3, 4);
}

TEST(sequence, add_multiple) {
    for(int i = 0; i < MAX_SEQUENCE_ITEMS; i++) {
        SS_sequence_add(i, 2*i, 6*i, 9*i);
    }
    for(int i = 0; i < MAX_SEQUENCE_ITEMS; i++) {
        check_sequence_values(i, i, 2*i, 6*i, 9*i);
    }
}
...
TEST(sequence, add_in_the_back) {
    SS_sequence_add(3, 3, 1200, 1000);
    SS_sequence_add(1, 1, 1000, 2000);
    SS_sequence_add(2, 2, 1100, 3000);
    check_sequence_values(0, 3, 3, 1200, 1000);
    check_sequence_values(1, 1, 1, 1000, 2000);
    check_sequence_values(2, 2, 2, 1100, 3000);
}
...
TEST(sequence, add_same_time) {
    SS_sequence_add(2, 2, 1100, 3000);
    SS_sequence_add(1, 1, 1000, 3000);
    SS_sequence_add(3, 3, 1200, 1000);
    check_sequence_values(0, 3, 3, 1200, 1000);
    check_sequence_values(1, 2, 2, 1100, 3000);
    check_sequence_values(2, 1, 1, 1000, 3000);
}
...
TEST(sequence, clear) {
    ...
    SS_sequence_clear();
    for(int i = 0; i < MAX_SEQUENCE_ITEMS; i++) {
        check_sequence_values(0, 0, 0, 0, 0);
    }
