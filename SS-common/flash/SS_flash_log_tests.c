...
static void open_file(lfs_file_t *file, struct lfs_file_config *cfg, uint8_t *buf, const char *path, int flags);
...
TEST(flash_log, reset_log_text)
{
    ...
    // No stop called to simulate reset.
...
static void open_file(lfs_file_t *file, struct lfs_file_config *cfg, uint8_t *buf, const char *path, int flags)
{
    ...
    lfs_t *lfs = SS_flash_lfs_get();
    TEST_ASSERT_EQUAL_INT(LFS_ERR_OK, lfs_file_opencfg(lfs, file, path, flags, cfg));
}
