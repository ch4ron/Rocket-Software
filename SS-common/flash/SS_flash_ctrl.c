...
static const uint32_t stream_max_len[FLASH_STREAM_COUNT] = {
    [FLASH_STREAM_VAR] = LOG_FILE_LEN,
    [FLASH_STREAM_TEXT] = LOG_FILE_LEN,
};

static const int32_t stream_direction[FLASH_STREAM_COUNT] = {
    [FLASH_STREAM_VAR] = 1,
    [FLASH_STREAM_TEXT] = -1,
};

static FlashStatus find_last_logged_page(FlashStream stream, uint32_t *page);
...
static volatile uint8_t page_buf1[FLASH_STREAM_COUNT][S25FL_PAGE_SIZE];
static volatile uint8_t page_buf2[FLASH_STREAM_COUNT][S25FL_PAGE_SIZE];
static volatile uint8_t *cur_page_buf[FLASH_STREAM_COUNT];
static volatile uint32_t cur_page_buf_pos[FLASH_STREAM_COUNT];
...
FlashStatus SS_flash_ctrl_init(void)
{
    ...
    for (uint32_t stream = 0; stream < FLASH_STREAM_COUNT; ++stream) {
    ...
    return FLASH_STATUS_OK;//SS_flash_ctrl_start_logging();
}

FlashStatus SS_flash_ctrl_update(void)
{
    /*for (uint32_t i = 0; i < FLASH_STREAM_COUNT; ++i) {
...
FlashStatus SS_flash_ctrl_start_logging(void)
{
    ...
    for (FlashStream stream = 0; stream < FLASH_STREAM_COUNT; ++stream) {
        FlashStatus status = find_last_logged_page(stream, (uint32_t *)&cur_page[stream]);
        if (status != FLASH_STATUS_OK) {
            return status;
        }
...
FlashStatus SS_flash_ctrl_stop_logging(void)
{
    ...
    for (FlashStream stream = 0; stream < FLASH_STREAM_COUNT; ++stream) {
        ...
        FlashStatus status = SS_s25fl_write_page_dma(cur_page[stream], (uint8_t *)cur_page_buf[stream]);
        if (status != FLASH_STATUS_OK) {
            return status;
        }
    }
...
FlashStatus SS_flash_ctrl_erase_logs(void)
{
    ...
    for (FlashStream stream = 0; stream < FLASH_STREAM_COUNT; ++stream) {
        uint32_t last_page;
        FlashStatus status = find_last_logged_page(stream, &last_page);
        if (status != FLASH_STATUS_OK) {
            return status;
        }
...
/*FlashStatus SS_flash_ctrl_write_pages_dma(uint32_t first_page, uint32_t len, uint8_t *data)
{
    FlashStatus status = FLASH_STATUS_OK;

    for (uint32_t i = 0; i < len; ++i, data += S25FL_PAGE_SIZE) {
        FlashStatus status = emulated_write_page_dma(first_page, data);
        if (status != FLASH_STATUS_OK) {
            return status;
        }
    }
...
FlashStatus SS_flash_ctrl_read_pages_dma(uint32_t first_page, uint32_t len, uint8_t *data)
{
    FlashStatus status = FLASH_STATUS_OK;
...
/*    return FLASH_STATUS_WRITE_PROTECTED;*/
/*}*/
...
/*        // Recurrent call. Be careful.*/
/*        FlashStatus status = find_last_logged_page(FLASH_STREAM_VAR, &last_logged_page);*/
/*        if (status != FLASH_STATUS_OK) {*/
/*            return status;*/
/*        }*/
...
/* FlashStatus SS_flash_ctrl_read_page_dma_wait(uint32_t page, uint8_t *data) */
/* { */
/*     FlashStatus status = SS_flash_ctrl_read_page_dma(page, data); */
/*     if (status != FLASH_STATUS_OK) { */
/*         return status; */
/*     } */
...
static FlashStatus log_byte(FlashStream stream, uint8_t byte)
{
    ...
    if (cur_page_buf_pos[stream] >= S25FL_PAGE_SIZE) {
        FlashStatus status = SS_s25fl_write_page(cur_page[stream], (uint8_t *)cur_page_buf[stream]);
        if (status != FLASH_STATUS_OK) {
            return status;
        }
