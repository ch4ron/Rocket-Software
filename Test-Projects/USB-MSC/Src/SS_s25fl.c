...
#define UNLOCK_THEN_RETURN(x) \
	if (unlock()) { \
		return x; \
	} else { \
		return S25FL_STATUS_ERR; \
	}
//#define UNLOCK(x) (unlock() ? (x) : S25FL_STATUS_ERR)
...
static S25flStatus get_status(void);
...
S25flStatus SS_s25fl_read_rems_id(uint16_t *id)
{
	...
	S25flStatus status = wait_until_ready(TIMEOUT_ms);
	if (status != S25FL_STATUS_OK) {
		UNLOCK_THEN_RETURN(status);
	}
...
S25flStatus SS_s25fl_wait_until_ready(void)
{
	...
	UNLOCK_THEN_RETURN(wait_until_ready(TIMEOUT_ms));
}
...
S25flStatus SS_s25fl_erase_all(void)
{
	...
	if (hal_status != HAL_OK) {
		UNLOCK_THEN_RETURN(translate_hal_status(hal_status));
	}
	...
	UNLOCK_THEN_RETURN(wait_until_ready(ERASE_ALL_TIMEOUT_ms));
}
...
S25flStatus SS_s25fl_erase_sector(uint32_t sector)
{
	...
	status = translate_hal_status(HAL_QSPI_Command(&hqspi, &cmd, TIMEOUT_ms));
	UNLOCK_THEN_RETURN(status);
}
...
S25flStatus SS_s25fl_write_bytes(uint32_t addr, uint8_t *data, uint32_t size)
{
	...
	UNLOCK_THEN_RETURN(cmd_write(cmd, data));
}
...
S25flStatus SS_s25fl_write_bytes_dma(uint32_t addr, uint8_t *data, uint32_t size)
{
	...
	QSPI_CommandTypeDef cmd = create_write_cmd(addr, size);
	UNLOCK_THEN_RETURN(cmd_write_dma(cmd, data));
}
...
S25flStatus SS_s25fl_read_bytes(uint32_t addr, uint8_t *data, uint32_t size)
{
	...
	UNLOCK_THEN_RETURN(cmd_read(cmd, data));
}
...
S25flStatus SS_s25fl_read_bytes_dma(uint32_t addr, uint8_t *data, uint32_t size)
{
	...
	QSPI_CommandTypeDef cmd = create_read_cmd(addr, size);
	UNLOCK_THEN_RETURN(cmd_read_dma(cmd, data));
}
...
S25flStatus SS_s25fl_read_bytes_dma_wait(uint32_t addr, uint8_t *data, uint32_t size)
{
	...
	return SS_s25fl_wait_until_dma_ready();*/
	...
	QSPI_CommandTypeDef cmd = create_read_cmd(addr, size);
	status = cmd_read_dma(cmd, data);
	if (status != S25FL_STATUS_OK) {
		UNLOCK_THEN_RETURN(status);
	}

	UNLOCK_THEN_RETURN(SS_s25fl_wait_until_dma_ready());
}
...
{
	/*S25flStatus status = SS_s25fl_read_page_dma(page, data);
	if (status != S25FL_STATUS_OK) {
		return status;
	}
...
static S25flStatus cmd_write_dma(QSPI_CommandTypeDef cmd, uint8_t *data)
{
	// This wait is likely superfluous.
	S25flStatus status = SS_s25fl_wait_until_dma_ready();
	if (status != S25FL_STATUS_OK) {
		return status;
	}

	// Note that this DMA write ready flag MUST be cleared right after waiting for DMA.
	// Otherwise concurrency bugs are possible.
...
static S25flStatus cmd_read_dma(QSPI_CommandTypeDef cmd, uint8_t *data)
{
	...
	// Note that this DMA read ready flag MUST be cleared right after waiting for DMA.
	// Otherwise concurrency bugs are possible.
...
static S25flStatus wait_until_ready(uint32_t timeout_ms)
{
	...
	while ((status = get_status()) == S25FL_STATUS_BUSY) {
		if (HAL_GetTick() - tickstart > timeout_ms) {
			return S25FL_STATUS_BUSY;
		}
	}
